...
describe('PaymentService', () => {
  ...
  it('should save a pix payment in DB', async () => {
  ...
  it('should save a credit card payment in DB', async () => {
    const spy = jest.spyOn(paymentRepository, 'save');
    const payment = await service.createPayment(
      createOrderCreditCardMock,
      [productMock],
  ...
  it('should return exception in not send a data', async () => {
    expect(
  ...
  it('should return final price send cartProduct', async () => {
    ...
    expect(savePayment.finalPrice).toEqual(1100);
  });
